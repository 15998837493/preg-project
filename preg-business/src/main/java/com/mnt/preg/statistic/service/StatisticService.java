
package com.mnt.preg.statistic.service;

import java.util.List;

import org.springframework.cache.annotation.CacheEvict;
import org.springframework.cache.annotation.CachePut;
import org.springframework.cache.annotation.Cacheable;

import com.mnt.preg.statistic.condition.SearchCountCondition;
import com.mnt.preg.statistic.condition.StatisticForm;
import com.mnt.preg.statistic.pojo.BirthEndingInfoPojo;
import com.mnt.preg.statistic.pojo.CustomerInfoPojo;
import com.mnt.preg.statistic.pojo.DiagnosisInfoPojo;
import com.mnt.preg.statistic.pojo.StatiscBirthResultPojo;
import com.mnt.preg.statistic.pojo.StatisticCustomerPojo;

/**
 * 数据统计平台
 * 
 * @author mlq
 * @version 1.0
 * 
 *          变更履历：
 *          v1.0 2018-08-14 mlq 初版
 */
public interface StatisticService {

    /**
     * 超级查询--查询患者列表
     *
     * @author zcq
     * @param condition
     * @return
     */
    List<StatisticCustomerPojo> queryCustomerList(StatisticForm condition);

    /**
     * 查询患者基本信息
     * 
     * @author mlq
     * @param condition
     * @return
     */
    List<CustomerInfoPojo> queryCustomerInfoByCustIds(SearchCountCondition condition);

    /**
     * 查询患者接诊信息
     * 
     * @author mlq
     * @param condition
     * @return
     */
    List<DiagnosisInfoPojo> queryDiagnosisInfoByCustIds(SearchCountCondition condition);

    /**
     * 查询患者分娩结局信息
     * 
     * @author mlq
     * @param condition
     * @return
     */
    List<BirthEndingInfoPojo> queryBirthEndingInfoByCustIds(SearchCountCondition condition);

    /**
     * 分娩结局统计
     * 
     * @author dhs
     * @param Date
     * @return
     */
    StatiscBirthResultPojo queryConfinedAccount(String resultStartDate, String resultEndDate);

    /**
     * 
     * [导出excel时查到的数据暂时放在缓存]
     *
     * @author lipeng
     * @param key
     * @param object
     * @return
     */
    @CachePut(value = "excelCache", key = "#key")
    public Object addExcelDataCache(String key, Object object);

    /**
     * 
     * [从缓存中获取数据]
     *
     * @author lipeng
     * @param key
     * @return
     */
    @Cacheable(value = "excelCache", key = "#key")
    public Object getExcelDataCache(String key);

    /**
     * 
     * [清除相应的缓存]
     *
     * @author lipeng
     * @param key
     */
    @CacheEvict(value = "excelCache", key = "#key")
    public void removeExcelDataCache(String key);
}
