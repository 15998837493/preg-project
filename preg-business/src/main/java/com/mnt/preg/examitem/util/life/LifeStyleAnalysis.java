 ...
 */
public class LifeStyleAnalysis extends NutritionAnalysis {

    public static final Logger LOGGER = LoggerFactory.getLogger(LifeStyleAnalysis.class);

    private LifeStyleItem lifeStyleItem;

    private String life_style_type = "life_style";

    private String meal = "meal";

    private String itemType = "cus_result_life";
    ...
    // 存放非报告分析结果
    private List<LifeStyleItem> lifeStyleItems = new ArrayList<LifeStyleItem>();

    // 返回非报告分析结果
    public List<LifeStyleItem> getLifeStyleItems() {
     ...
     * @return
     */
    protected LifeStyleItem createLifeStyleItem(String recordId, String item, String itemString) {
        lifeStyleItem = new LifeStyleItem();
        lifeStyleItem.setExamId(recordId);
        lifeStyleItem.setItemCode(item);
        lifeStyleItem.setItemName(lifeStyleConstant.getLifeStyleItem(item));
        lifeStyleItem.setItemType(itemType);
        if (StringUtils.isEmpty(itemString) || "0".equals(itemString)) {
            itemString = "——";
        }
        lifeStyleItem.setItemString(itemString);
        lifeStyleItem.setItemClassify(life_style_type);
     ...
     * @return
     */
    protected LifeStyleItem createLifeStyleItem(String recordId, String item, String itemString, String itemException) {
        lifeStyleItem = new LifeStyleItem();
        lifeStyleItem.setExamId(recordId);
        lifeStyleItem.setItemCode(item);
        lifeStyleItem.setItemName(lifeStyleConstant.getLifeStyleItem(item));
        ...
        lifeStyleItem.setItemString(itemString);
        lifeStyleItem.setItemClassify(life_style_type);
        lifeStyleItem.setItemResult(itemException);
        return lifeStyleItem;
    }
     ...
     * @return
     */
    protected LifeStyleItem createLifeStyleItem(String recordId, String item, String itemString, String unit,
        ...
        lifeStyleItem.setItemString(itemString);
        lifeStyleItem.setItemUnit(unit);
        lifeStyleItem.setItemCompare(itemCompare);
        lifeStyleItem.setItemRefValue(itemRefValue);
        String itemException = "";
        if (StringUtils.isNotEmpty(itemCompare)) {
            if (up.equals(itemCompare)) {
                if (lifeStyleConstant.cbrratio.equals(item) || lifeStyleConstant.protidratio.equals(item)
                        || lifeStyleConstant.fatratio.equals(item)) {
                    itemException = lifeStyleItem.getItemName() + "超标";
                } else {
                    itemException = lifeStyleItem.getItemName() + excessResult;
                }
            }
            if (down.equals(itemCompare)) {
                if (lifeStyleConstant.cbrratio.equals(item) || lifeStyleConstant.protidratio.equals(item)
                        || lifeStyleConstant.fatratio.equals(item)) {
                    itemException = lifeStyleItem.getItemName() + "不足";
                } else if (lifeStyleConstant.water.equals(item)) {
                    itemException = lifeStyleItem.getItemName() + "小于1500ml";
                } else {
                    itemException = lifeStyleItem.getItemName() + lackingResult;
                }
            }
        }
        lifeStyleItem.setItemResult(itemException);
        lifeStyleItem.setItemClassify(meal);
        return lifeStyleItem;
    }
     ...
     */
    protected LifeStyleItem createLifeStyleItem(String recordId, String item, String itemString, String unit,
            String itemCompare, String itemResult, String itemException, String itemRefValue) {
        lifeStyleItem = new LifeStyleItem();
        lifeStyleItem.setExamId(recordId);
        lifeStyleItem.setItemCode(item);
        lifeStyleItem.setItemName(lifeStyleConstant.getLifeStyleItem(item));
        ...
        lifeStyleItem.setItemString(itemString + unit + itemResult);
        lifeStyleItem.setItemUnit(unit);
        lifeStyleItem.setItemCompare(itemCompare);
        lifeStyleItem.setItemResult(itemException);
        lifeStyleItem.setItemRefValue(itemRefValue);
    ...
    protected LifeStyleItem createLifeStyleItem(String recordId, String item, String itemString, String unit,
        ...
        lifeStyleItem.setItemResult(itemException);
        lifeStyleItem.setItemRefValue(itemRefValue);
        lifeStyleItem.setItemClassify(classify);
        return lifeStyleItem;
    }

    public LifeStyleAnalysis(List<LifeStyleQuestionAnswerPojo> lifeStyleQuestionAnswerBos,
    ...
    public String getMeatRatio() {
        ...
        if (ratio >= 50) {
            ...
            lifeStyleItems.add(createLifeStyleItem(recordId, lifeStyleConstant.B00576, ratio + "%", "", "", up,
                    lifeStyleConstant.getLifeStyleItem(lifeStyleConstant.B00576) + ":" + ratio + "%", "50%"));
        }
        if (0 < ratio && ratio < 50) {
     ...
     * @return
     */
    public String getLackingElement(String diagnosisPregnantStage) {
        StringBuffer result = new StringBuffer();
        double standard = 0.9;
        // 1 判断碘是否显示
        if (CollectionUtils.isNotEmpty(dietQuestionAnswerBos)) {
            ...
            if (iodine.indexOf(lifeStyleConstant.O11002) == -1) {
                ...
                lifeStyleItems.add(createLifeStyleItem(recordId, lifeStyleConstant.B00524, "", "",
        ...
        if (CollectionUtils.isEmpty(p107List)) {
            if (deepFish < 50) {
                ...
                lifeStyleItems.add(createLifeStyleItem(recordId, lifeStyleConstant.B00525, "", "",
                        lifeStyleConstant.exception, down, ""));
            }
        ...
        // 蛋白质总量
        double foodProtidweight = getTotalContent(foodProtid, false);
        // 优质蛋白质总量
        double goofProtidweight = getGoodProtid(foodProtid);
        if (goofProtidweight < foodProtidweight * 0.5) {
        ...
        if (foodDfweight < 24) {
            ...
            lifeStyleItems.add(createLifeStyleItem(recordId, lifeStyleConstant.B00509,
        ...
        if (foodEcaweight < recomendfoodEca) {// mg
            ...
            lifeStyleItems.add(createLifeStyleItem(recordId, lifeStyleConstant.B00519,
        ...
        if (foodEkweight < recomendfoodEk) {// mg
            ...
            lifeStyleItems.add(createLifeStyleItem(recordId, lifeStyleConstant.B00520,
        ...
        if (foodEmgweight < recomendfoodEmg) {// mg
            ...
            lifeStyleItems.add(createLifeStyleItem(recordId, lifeStyleConstant.B00521,
        ...
        if (foodEfeweight < recomendfoodEfe) {// mg
            ...
            lifeStyleItems.add(createLifeStyleItem(recordId, lifeStyleConstant.B00522,
        ...
        if (foodEznweight < recomendfoodEzn) {// mg
            ...
            lifeStyleItems.add(createLifeStyleItem(recordId, lifeStyleConstant.B00523,
        ...
        if (foodVaweight < recomendfoodVa) {// mg
            ...
            lifeStyleItems.add(createLifeStyleItem(recordId, lifeStyleConstant.B00510,
        ...
        if (foodVcweight < recomendfoodVc) {// mg
            ...
            lifeStyleItems.add(createLifeStyleItem(recordId, lifeStyleConstant.B00511,
        ...
        if (foodVeweight < recomendfoodVe) {// mg
            ...
            lifeStyleItems.add(createLifeStyleItem(recordId, lifeStyleConstant.B00512,
        ...
        if (foodAfweight < recomendfoodAf) {// mg
            ...
            lifeStyleItems.add(createLifeStyleItem(recordId, lifeStyleConstant.B00515,
        ...
        if (foodVb6weight < recomendfoodVb6) {// mg
            ...
            lifeStyleItems.add(createLifeStyleItem(recordId, lifeStyleConstant.B00517,
        ...
        if (foodVb12weight < recomendfoodVb12) {// mg
            ...
            lifeStyleItems.add(createLifeStyleItem(recordId, lifeStyleConstant.B00518,
        ...
        if (foodVb9weight < recomendfoodVb9) {// mg
            ...
            lifeStyleItems.add(createLifeStyleItem(recordId, lifeStyleConstant.B00516,
    ...
    public String getActualTotalEnergy() {
        ...
        result = String.valueOf((int) min) + "~" + String.valueOf((int) max);
        return result;
    }
    ...
    public String getRecommendProtid(double custWeight) {
        ...
        String result = (int) Math.rint(custWeight) + "~" + (int) custWeight * 2;
    ...
    public String getRecommendFat(double custWeight, String pregnantStage) {
        ...
        result = (int) Math.rint(recommendEnergy * 0.2 / 9) + "~" + (int) Math.rint(recommendEnergy * 0.3 / 9);
        return result;
    }
    ...
    public void getLifeStyleItem(String explain, String problemId, String type, String itemCode) {
        ...
        if (CollectionUtils.isNotEmpty(questions)) {
            ...
            if (result.length() > 0) {
                ...
                lifeStyleItems.add(createLifeStyleItem(recordId, itemCode, explain + result.toString(), "",
                        lifeStyleConstant.exception, "", explain + result.toString(), "", life_style_type));
            }
        }
    ...
    public void addP103Result() {
        ...
        if (result.length() > 0) {
            lifeStyleItems.add(createLifeStyleItem(recordId, lifeStyleConstant.B00539,
                    result.substring(result.length() - 1).toString(), "", lifeStyleConstant.exception, "", result
                            .substring(result.length() - 1).toString(), "", life_style_type));
        }
