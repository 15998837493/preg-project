...
function deleteDiseaseItem(id){
	...
	function(){
		var url = URL.get("item.DELETE_INSPECT");
    	var params = "id=" + id;
...
//弹出窗中添加链接的方法
function addInspect(inspectId){
	var itemConfigIds = '';
	$("input:radio[name='tableRadio']").each(function(index, obj){
		itemConfigIds += $(obj).val(); //这里的value就是每一个input的value值~
	});
	//判断指不存在的话，添加指标
	if(itemConfigIds.indexOf(inspectId) < 0){
		var url = URL.get("item.ADD_INSPECT");
		var params = "inspectId=" + inspectId + "&diseaseId=" + $("#diseaseIdQuery").val();
		ajax.post(url,params,dataType.json,function(data){
			datatable.add(quotaTable, data.value);// 添加
			setDH();
		});
	}else{
		layer.msg('不能重复添加相同的项目！');
	}
}