 ...
 * @param deptId
 */
function getDoctorInspectListJson(doctorId) {
    if (!_.isEmpty(doctorId)) {
        // 选取医生所属默认项目
        ajax.post(URL.get("Evaluate.DOCTOR_INSPECT_LIST") + "?inspectType=first", "doctorId=" + doctorId, dataType.json, function (data) {
            $("#inspectTable").empty();
...
function addRegistration() {
    ...
    //接诊医生姓名赋值
    $("#diagnosisUserName").val($("#diagnosisUser option:selected").text());
    // 保存信息
...
function diagnosisVist(custId, diagnosisType) {
    ...
    if (diagnosisType == "3") {
        // 初始化接诊医生
        ajax.post(URL.get("Platform.DIAGNOSIS_LAST_INFO"), "custId=" + custId, dataType.json, function (data) {
            if (!_.isEmpty(data.value)) {
                // 初始化接诊医生
                var list = data.value.diagnosisUser;
                if (!common.isEmpty(list)) {
                    $(list).each(function (index, right) {
                        $("option[name='diagnosisUserEdit'][value='" + right.rightId + "']").attr("checked", true);
                    });
                }
            }
        }, false, false);
        $("#editDiagnosisUserModal").modal("show");
    ...
    if (diagnosisType == "2") {
        ...
        ajax.post(URL.get("Platform.DIAGNOSIS_LAST_INFO"), "custId=" + custId, dataType.json, function (data) {
            if (!_.isEmpty(data.value)) {
                ...
                if (!common.isEmpty(list)) {
                    $(list).each(function (index, right) {
                        $("option[name='diagnosisUserVist'][value='" + right.rightId + "']").attr("checked", true);
                    });
                }
            }
        }, false, false);
...
function editDiagnosis() {
    //接诊信息添加接诊医生id、姓名
    var diagnosisJson = {
        diagnosisCustomer: $("#custIdEdit").val(),
        diagnosisUser: $("#diagnosisUserEdit option:selected").val(),
        diagnosisUserName: $("#diagnosisUserEdit option:selected").text(),
        diagnosisAssistantStatus: "1",
        diagnosisType: "3",
        diagnosisOrg: "孕期营养门诊",
    };
    var url = URL.get("Platform.DIAGNOSIS_ADD");
    var param = "diagnosisJson=" + JSON.stringify(diagnosisJson);
    $("#editDiagnosisUserModal").modal("hide");
...
function vistDiagnosis() {
    ...
    var diagnosisJson = {
        ...
        diagnosisUser: $("#diagnosisUserVist option:selected").val(),
        diagnosisUserName: $("#diagnosisUserVist option:selected").text(),
        diagnosisAssistantStatus: "1",
        diagnosisType: "2",
...
var tableOptions = {
    ...
    columns: [
        ...
        {"data": "diagnosisUserName", "sClass": "text-center", "orderable": false},
        {
            "data": null, "sClass": "text-center", "orderable": false,
            "render": function (data, type, row, meta) {
                return "<a onclick='toGuidePage(\"" + data.diagnosisId + "\");'>等待接诊</a>";
            }
        }
    ],
    rowClick: function (data, row) {
...
var admissionsTableOptions = {
    ...
    columns: [
        ...
        {"data": "diagnosisUserName", "sClass": "text-center", "orderable": false},
        {
            ...
            "render": function (data, type, row, meta) {
                var content = "";
                if (common.isEmpty(data.diagnosisAssistantStatus) || data.diagnosisAssistantStatus == 2) {
                    content = "<span style='color:red;'>接诊</span>";
                } else if (data.diagnosisAssistantStatus == 3) {
                    content = "完成";
                }
                return "<a onclick='toGuidePage(\"" + data.diagnosisId + "\");'>" + content + "</a>";
            }
        }
    ],
    rowClick: function (data, row) {
...
function toGuidePage(diagnosisId) {
    ...
    ajax.post(url, null, dataType.json, function (data) {
        if (data.value) {
            window.open(URL.get("Evaluate.EVALUATE_GUIDE_PAGE") + "?diagnosisId=" + diagnosisId);
        } else {
            refreshDataTable();
            common.openWindow(URL.get("Customer.DIAGNOSIS_INITAL_VIEW_GET") + "?openerType=assistant&diagnosisId=" + diagnosisId);
        }
    }, false, false);
}
    ...
    if ($("#" + tableId + "_" + inspect.inspectId).length == 0) {
        if (checked) {
            $("#" + tableId).append(
                ...
                "	<label class='label-checkbox' style='padding-left:8px;'><input type='checkbox' style='display:none;' id='inspect-checkbox' inspectCode='" + inspect.inspectCode + "' inspectName='" + inspect.inspectName + "' value='" + inspect.inspectId + "' checked>" + inspect.inspectName + "</label>" +
            ...
            $("#" + tableId).append(
                "<tr id='" + tableId + "_" + inspect.inspectId + "'><td>" +
                "	<label class='label-checkbox' style='padding-left:8px;'><input type='checkbox' style='display:none;' id='inspect-checkbox' inspectCode='" + inspect.inspectCode + "' inspectName='" + inspect.inspectName + "' value='" + inspect.inspectId + "'>" + inspect.inspectName + "</label>" +
                "	<a onclick='removeTdHtml(\"" + tableId + "_" + inspect.inspectId + "\")' style='float: right; color: red;'><i class='fa fa-remove fa-fw'></i></a>" +
                "</td></tr>"
            );
        }
...
$().ready(function () {
    ...
    $("#diagnosisUserEdit").addClass("inputborder");//随诊接诊医生
    $("#diagnosisUserVist").addClass("inputborder");//复诊接诊医生
    $("#diagnosisReferralDoctorVist").addClass("inputborder");

    // 首诊登记弹出框关闭时触发事件
    // 首诊登记默认不加载评价项目，视所选医生而定
    //initUserInspectItem("inspectTable", "inspectItem");
