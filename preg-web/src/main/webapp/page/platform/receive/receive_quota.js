...
function addReportHtml(report){
	...
	var reportHtml = 
			...
			"			<input type='text' class='form-control' placeholder='请输入检验项目' id='"+reportId+"_item' disabled/>"+
			...
			"           <input type='text' class='form-control' placeholder='检验报告名称' style='width:18%;' id='"+reportId+"_name' value='"+reportName+"' onblur='changeReportInfo(this,\""+reportId+"\");'/>"+
			"	        <div class='input-group'>"+
			"	            <input id='"+reportId+"_date' class='form-control' placeholder='请选择检验日期' style='background-color: white;' onkeypress='if(event.keyCode==8)' onchange='changeReportInfo(this,\""+reportId+"\");'>"+
			...
			"						<th class='text-center' colspan='2' style='width:31%;'>检验项目</th>"+
			"						<th class='text-center' style='width:10%;'>结果</th>"+
			"						<th class='text-center' style='width:10%;'>结论</th>"+
			"						<th class='text-center' style='width:11%;'>上次孕周</th>"+
			"						<th class='text-center' style='width:11%;'>上次结果</th>"+
			"						<th class='text-center' style='width:9%;'>单位</th>"+
			"						<th class='text-center' style='width:11%;'>参考范围</th>"+
			"				</thead>"+
			"				<tbody></tbody>"+
			"			</table>"+
			"		</div>"+
			...
			"		</div>"+
			"	</div>"+
			"	<div class='panel-body col-xs-12' id='"+reportId+"_itemInferDiv' style='display: none'></div>"+
			"</div>";
	...
	if(!common.isEmpty(report.itemList)){
		...
		$("#"+reportId+"_item1 tbody").append("<tr name='noItemTr'><td colspan='9' style='text-align: center;'><h4>请选择检验项目！</h4></td></tr>");
	}

	//如果添加进来的报告单状态是2,那么禁用指定报告单
	if(report.reportStatus == 2){
		$("#"+reportId).disable();
		$("#"+reportId+" a").css("pointer-events","none");
	}
	// 解决样式不生效  
...
function addLastInspectsHtml(inspect){
	...
	$("#lastInspects").append(
		"<tr><td>"+
		"	<label class='label-checkbox' style='padding-left:8px;'><input type='checkbox' style='margin-right:10px;' class='inspect-checkbox' name='"+inspectName+"' value='"+inspectId+"'>"+inspectName+"</label>"+
		"</td></tr>");
}
...
function saveItems(items,inspect,reportId){
	...
	ajax.post(url, params, dataType.json, function(data){
		if(!common.isEmpty(data.value)){
			...
			$("#"+reportId+"_items tbody").empty();//清空报告单检验项目信息  重新获取数据
			//超过固定条数  分两表显示
			...
			if($("#"+reportId+"_items").hasClass("hiddenClass")){
				$("#"+reportId+"_items").removeClass("hiddenClass");
			}
...
function addItemHtml(index,sumLength,item,reportId){
	...
	if(itemMap.has(inspectId+"_"+itemId)){
		...
		var gestationalWeeks = "————";
		if(item.gestationalWeeks != null && item.gestationalWeeks.length > 0){
			week = item.gestationalWeeks.split("+")[0];
			day = item.gestationalWeeks.split("+")[1];
			gestationalWeeks = week+' <sup style="font-size: 85%;">+'+day+'</sup>';
		}
		var lastValue = item.lastItemValue || "";
		var lastResult = item.lastItemResult || "";
		var lastValueResult = (lastValue+lastResult) || "————";
		...
		var itemHtml = 
			...
			"   <input type='hidden' name='"+reportId+"_"+itemId+"_lastResult' value='"+lastResult+"'/>" +
			"	<td class='text-center' style='border-left:0;'>"+
			"		<div style='text-align: center;'>"+(index+1)+"</div></td>"+
			"	<td style='width: 30%;border-right:0;' class='item-name'>"+
			"		<a data-html='true' data-toggle='tooltip' data-placement='top' data-title='' onclick='showItemHistroy(this,\""+itemId+"\")'>"+item.itemName+"</a></td>"+
			"	<td style='width: 1%;border-left:0;'>"+
			"		<a class='text-right redClass' onclick='removeItem(\""+reportId+"\",\""+inspectId+"\",\""+itemId+"\")'>X</a></td>"+
			"	<td style='text-align: center;'>"+
			"		<input type='text' class='intake-sm enter-input' style='width:100%;' name='"+reportId+"_"+itemId+"_itemValue' value='"+itemValue+"' maxlength='30' onkeypress='if(event.keyCode==13) focusNextInput(this,\""+reportId+"\");' onchange='changeItemResult(this,\""+reportId+"\",\""+inspectId+"\",\""+itemId+"\");'></td>"+
			"	<td style='text-align: center;'>"+
			"		<input type='text' class='intake-sm' style='width:100%;' name='"+reportId+"_"+itemId+"_itemResult' value='"+itemResult+"' maxlength='1000' onblur='updateItem(\""+reportId+"\",\""+itemId+"\");'></td>"+
			"	<td>"+
			"		<div style='text-align: center;'>"+gestationalWeeks+"</div></td>"+
			"	<td>"+
			"		<div style='text-align: center;'>"+lastValueResult+"</div></td>"+
			"	<td>"+
			"		<div style='text-align: center;'>"+itemUnit+"</div></td>"+
			"	<td>"+
			"		<div style='text-align: center;'>"+itemRefValue+"</div></td>"+
			"</tr>";
		...
		if(!common.isEmpty(itemResult)){
			if("↓"==itemResult || "↑"==itemResult){
				$("#"+reportId+"_"+itemId+" a").addClass("red");
				$("#"+reportId+"_"+itemId+" div").addClass("red");
			}
		}
		
		//本次结论正常，上次结论异常  显示为绿色
		if(!common.isEmpty(itemResult) && !common.isEmpty(lastResult)){
			if(("↓"!=itemResult && "↑"!=itemResult) && ("↓"==lastResult || "↑"==lastResult)){
				$("#"+reportId+"_"+itemId+" a").addClass("green");
				$("#"+reportId+"_"+itemId+" div").addClass("green");
			}
		}
	}
}
 ...
 * @param itemId
 */
function showItemHistroy(obj, itemId){
	var url = URL.get("Platform.DIAGNOSIS_HOSPITAL_ITEM_QUERY");
	var params = "custId=" + $("#custId").val() + "&itemId=" + itemId;
	ajax.post(url, params, dataType.json, function(data){
		var hisItemList = data.value;
		if(!common.isEmpty(hisItemList)){
			var itemHisDetail = "";
			for ( var i = 0; i < hisItemList.length; i++) {
				var itemHis = hisItemList[i];
				if(!common.isEmpty(itemHis.reportDate)){
					var preg_week = pregnancy.getGestWeeksByDueDate(itemHis.reportDate, preArchiveJson.pregnancyDueDate);
					if(preg_week != null && preg_week.length > 0){
						week = preg_week.split("+")[0];
						day = preg_week.split("+")[1];
						preg_week = week+' <sup style="font-size: 85%;">+'+day+'</sup>';
					}
					
					var s = "";
					if("↓"==itemHis.itemResult || "↑"==itemHis.itemResult){
						s = 'style="color:red;"'
					}
					itemHisDetail += `
						<tr ${s}>
							<td class="text-center">${itemHis.reportDate || ''}</td>
							<td class="text-center">${preg_week || ''}</td>
							<td class="text-center">${itemHis.itemValue || ''}</td>
							<td class="text-center">${itemHis.itemResult || ''}</td>
						</tr>
					`
				}
			}
			var itemHisTable = `
				<table style="border:1px;width:200px;margin-left:-5px;">
					<thead>
						<tr>
					...
					<tbody>${itemHisDetail}</tbody>
				</table>
			`
			$(obj).attr('data-title', itemHisTable);
			$(obj).tooltip("show");
		}else{
			layer.alert("历史记录未找到！");
		}
...
function changeItemResult(obj,reportId,inspectId,itemId){
	// 判断检验日期是否填写
	if(common.isEmpty($("#"+reportId+"_date").val())){
	...
	var lastResult = $("input[name='"+reportId+"_"+itemId+"_lastResult']").val();
	var itemResult = $("input[name='"+reportId+"_"+itemId+"_itemResult']").val();
	if("↓"==itemResult || "↑"==itemResult){
		$("#"+reportId+"_"+itemId+" a").removeClass("green").addClass("red");
		$("#"+reportId+"_"+itemId+" div").removeClass("green").addClass("red");
	}else if(!common.isEmpty(itemResult) && ("↓"==lastResult || "↑"==lastResult)){
		$("#"+reportId+"_"+itemId+" a").removeClass("red").addClass("green");
		$("#"+reportId+"_"+itemId+" div").removeClass("red").addClass("green");
	}else{
		$("#"+reportId+"_"+itemId+" a").removeClass("red green");
		$("#"+reportId+"_"+itemId+" div").removeClass("red green");
	}
 ...
 * 由检验项目推断诊断项目
 */
function queryFitItem(){
	var url = URL.get("Platform.DISEASE_ITEM_GROUP_FIT");
	...
	ajax.post(url, params, dataType.json, function(data){
		...
		if(data.value){
			$(data.value).each(function(index, disease){
				addDiseaseButton(disease.diseaseName);
			});
			$("#itemInferDiv").show();
		}
	}, false, false, url);
 ...
 * @param diseaseName
 */
function addDiseaseButton(diseaseName){
	$("#itemInferDiv").append(
		'<div class="btn-group btn-group-right">'+
			'<button class="btn btn-lightblue btn-width" type="button">'+diseaseName+'</button>'+
			'<button type="button" class="btn btn-lightblue" onclick="definiteDis(\''+diseaseName+'\')">确认</button>'+
		'</div>'
	);
}
 ...
 * @param diseaseName
 */
function definiteDis(diseaseName){
	saveDiagnosisDisease(diseaseName, '1');
}
/**************************************************************诊断项目模块end**************************************************************/
 ...
 * @param date
 */
function pregWeekHTMLSup(date){
	return common.isEmpty(date)?'':pregnancy.gestationalWeeksSupHtml(pregnancy.getGestWeeksByDueDate(date, preArchiveJson.pregnancyDueDate));
}
