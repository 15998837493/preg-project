...
(function() {
	...
	// An internal function for creating a new object that inherits from
	// another.
	...
	function createReduce(dir) {
		...
		return function(obj, iteratee, memo, context) {
			...
			var keys = !isArrayLike(obj) && _.keys(obj), length = (keys || obj).length, index = dir > 0 ? 0
					: length - 1;
	...
	// [Fisher-Yates
	// shuffle](http://en.wikipedia.org/wiki/Fisher鈥揧ates_shuffle).
	...
	_.bind = function(func, context) {
		...
		var args = slice.call(arguments, 2);
		var bound = function() {
			return executeBound(func, bound, context, this, args.concat(slice
					.call(arguments)));
		};
		return bound;
	};
	...
	_.partial = function(func) {
		...
		var bound = function() {
			...
			for ( var i = 0; i < length; i++) {
				args[i] = boundArgs[i] === _ ? arguments[position++]
						: boundArgs[i];
			}
	...
	function collectNonEnumProps(obj, keys) {
		...
		var proto = (_.isFunction(constructor) && constructor.prototype)
				|| ObjProto;
